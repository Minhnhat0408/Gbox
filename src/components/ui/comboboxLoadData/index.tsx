...
import React, { ChangeEvent, UIEvent, useEffect, useLayoutEffect, useRef, useState } from "react";
...
interface ComboboxControl {
    ...
    getCurrentData:   (currentPage: number, textSearch: string) => Promise<Array<any>>
}
...
export function useComboboxLoadData(valueField: string, displayField: string, getCurrentData: (currentPage: number, textSearch: string) => Promise<Array<any>>) : ComboboxControl {
...
const comboboxLoadData = React.forwardRef<HTMLElement, ComboboxLoadDataProps>(({comboboxControl}, ref) => {
    ...
    const {delayValue: debounceTextSearch, isDelaying} = useDebounce<string>(textSearch, 500);
    const previousDebounce = usePrevious<string>(debounceTextSearch)
    ...
    async function getData(currentPage: number, query: string) : Promise<Array<any>> {
        setIsLoading(true)
    ...
    function eventScrollData(e : UIEvent<HTMLDivElement>) {
        const isMaxScroll = e.currentTarget.scrollHeight == e.currentTarget.scrollTop + e.currentTarget.clientHeight
    ...
    return <div className="w-[500px] ml-5 relative">
        ...
        <div ref={refBodyCombo} onScroll={eventScrollData} className={`absolute bg-white top-[calc(100%+5px)] left-0 w-full max-h-[200px] rounded-sm overflow-x-hidden overflow-y-auto ${style['custom-scrollbar']} ${isOpenCombo? 'block': 'hidden'}`}>
            {isLoading == false? data.length > 0 ? data.map(i => {
                return <div key={i[comboboxControl.valueField]} onClick={() => handleSetData(i)} className={`flex px-3 py-1 justify-between items-center hover:bg-slate-300 cursor-pointer ${comboboxControl.value == i[comboboxControl.valueField] && 'bg-slate-300'}`}>
                    <span className="text-black text-sm">{i[comboboxControl.displayField]}</span>
                </div>
            }) :<div className="text-black text-center font-semibold">
                    No data found
                </div> : <div className="h-[200px] text-black bg-white flex items-center justify-center">
