...
export default function RoomShortCut() {
  ...
  return (
    <button
      ...
      className={cn(
        "fixed bottom-10 left-1/2 fancy -translate-x-1/2 py-3 px-6 bg-muted flex justify-center  rounded-full ",
        roomData?.matching_time && "bg-black/40 fancy-2"
      )}
    >
      <span
        className={cn(
          "font-bold z-10 tracking-wider  ",
          roomData?.matching_time ? "text-[#00d9f5]" : "super"
        )}
      >
