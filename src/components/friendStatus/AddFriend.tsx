...
export default function AddFriend({ search, currentUserID, id }: { search: string, currentUserID: string, id: string }) {
  ...
  return (
    <button className="rounded-lg w-[170px] flex items-center justify-center h-10 text-[1rem] bg-[#3dbda7]"
      onMouseDown={async () => {
        ...
        await axios.post(`/api/friends/sendFriendReqs?id=${currentUserID}&receiverID=${id}`);
